...
local function loadConfigModule()
    local direct = ReplicatedStorage:FindFirstChild("Config")
    if direct and direct:IsA("ModuleScript") then
        return require(direct)
    end

    local modulesFolder = ReplicatedStorage:FindFirstChild("Modules")
    local nested = modulesFolder and modulesFolder:FindFirstChild("Config")
    if nested and nested:IsA("ModuleScript") then
        return require(nested)
    end

    warn("[TrashSim] Config ModuleScript not found. Create a ModuleScript named 'Config' under ReplicatedStorage (or ReplicatedStorage/Modules).")
    return nil
end

local Config = loadConfigModule()
if not Config then
    return
end
